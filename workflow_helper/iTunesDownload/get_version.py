...
def getAppInfo(appUrl, storeFront):
    appInfo = requests.get(appUrl, headers={"X-Apple-Store-Front": STORE_TABLE[storeFront.upper()]}).text
    appParam = re.findall(r'"buyParams":"(.*?)"', appInfo)[0]
    appParamDict = dict((c.split('=') for c in json.loads('"%s"' % appParam).split('&')))
    appId = appParamDict['salableAdamId']
...
if __name__ == '__main__':
    import sys
    args = sys.argv[1:]
    appId, appVer = getAppInfo(args[0], args[1])
    print("AppId: %s AppVer: %s" % (appId, appVer))